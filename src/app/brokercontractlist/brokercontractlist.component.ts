...
export class BrokercontractlistComponent implements OnInit {
  ...
  brokercontractlist(){
       ...
       this.http.get(localStorage.getItem("LandRegistryURL")+"getBrokerContractDetailsToAddBuyer/"+localStorage.getItem("LandRegistryUserAddress"))
       .subscribe(
         d=>{
   ...
   addBuyer(){
    ...
    "buyerAddress":this.sellerAddress
    })
    .subscribe(
      d=>{
   ...
   getBuyerList(){
       this.http.get(localStorage.getItem("LandRegistryURL")+"getBuyersList/")
       .subscribe(
         d=>{
           console.log("user contract #########################")
           let dt = JSON.parse(JSON.stringify(d));
